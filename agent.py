...
def run_travel_agent(user_prompt: str, callback_log=None) -> dict:
    """
    Runs the LangChain Google GenAI ToolCalling Agent.
    """
    api_key = os.getenv("GOOGLE_API_KEY") or os.getenv("GEMINI_API_KEY")
    if not api_key:
        return {
            "success": False,
            "itinerary": "Error: GOOGLE_API_KEY or GEMINI_API_KEY is not set in environment or sidebar.",
            "traces": []
        }
    
    # Initialize the LLM via LangChain Google GenAI (using robust gemini-3.5-flash)
    ...
    # Initialize Google GenAI Tools Agent runnable
...
if __name__ == "__main__":
    ...
    api_to_test = os.getenv("GOOGLE_API_KEY") or os.getenv("GEMINI_API_KEY")
    if not api_to_test:
        print("Please set your GOOGLE_API_KEY (or GEMINI_API_KEY) in .env to run this standalone script.")
    else:
        print("Running LangChain trip travel agent simulation with Google GenAI...")
